...
**The most important thing to understand before adding anything.** Recast offers
...
Slides have no hub on purpose: Recast reads a deck as text and writes one from
...
**`lib/registry/table.ts` — the edges.** What Recast converts in one step, and how
...
| `unsupported`         | `index.ts`   | Pairs Recast deliberately refuses, with reasons.   |
...
| `fail(message, cause?)`        | Throws a `RecastError` — a message shown to the user verbatim.    |
...
- **Never reach the network.** No CDN, no hosted API, no font fetch. A WASM blob
  from Recast's own origin is fine; anything else is disqualified however good its
  output.
...
A pair Recast does not currently declare, built completely.
...
`sample.rtf` already exists, generated by `scripts/make-fixtures.mjs` from Recast's
