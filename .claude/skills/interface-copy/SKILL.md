---
name: interface-copy
description: Use when writing or changing any words a person reads in Kiln — a button, a label, an empty state, a warning under a finished conversion, an error message, a pair's caveat in the registry table, a reason in the unsupported list, or the front-page copy. Also use when reviewing copy someone else wrote, when a warning reads oddly in context, or when deciding what a failure should say.
---

# Writing Kiln's words

Kiln's copy carries the product's one promise and every one of its limits. A
...
- **Active voice**, and say who did what. "Kiln went with the contents", not "the
  contents were used".
...
of Kiln's 114 pairs are two converters composed, and **each step's caveat and
...
wrote that Markdown — and here Kiln wrote it, from a deck.
...
same sentence written badly and well, the warning and error patterns Kiln
